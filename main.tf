...
resource "azurerm_container_app" "spiritops_app" {
  ...
  resource_group_name = azurerm_resource_group.aicloudbuilder.name
  location            = var.location
  managed_environment_id = azurerm_app_service_environment.spiritops_container_app_env.id
}

resource "azurerm_app_service_environment" "spiritops_container_app_env" {
